...
export default async function ValorantHome() {
  ...
  const activeSeason = await (prisma as any).season.findFirst({
    where: { isActive: true, game: Game.VALORANT }
  });
  ...
  const allGameStats = await prisma.playerGameStat.findMany({
    ...
    include: {
      player: {
        include: {
          matchParticipants: {
            where: { match: { game: Game.VALORANT } }
          }
        }
      }
    }
  ...
  return (
    <div className="space-y-12 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* HERO SECTION */}
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-red-600/10 via-background to-red-400/5 p-6 md:py-8 md:px-12 border border-red-500/20 shadow-sm mt-4">
        <div className="absolute top-0 right-0 -mt-20 -mr-20 w-64 h-64 bg-red-500/20 rounded-full blur-3xl opacity-50 pointer-events-none"></div>
        <div className="relative z-10 w-full flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-4">
            <div className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[10px] uppercase font-black tracking-wider shadow-sm w-fit ${activeSeason ? "border-red-500/20 bg-red-500/10 text-red-500" : "border-destructive/20 bg-destructive/10 text-destructive"}`}>
              <span className={`flex h-1.5 w-1.5 rounded-full mr-2 animate-pulse ${activeSeason ? "bg-red-500" : "bg-destructive"}`}></span>
            ...
            <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-foreground leading-[1.1]">
              Valorant <span className="text-red-500 bg-clip-text text-transparent bg-gradient-to-r from-red-500 to-red-400/60">Arena</span>
            </h1>
            <p className="text-base md:text-lg text-muted-foreground max-w-xl leading-relaxed">
              Taktiksel atışlarını sergile, clutch at ve radiant zirvesine çık!
            </p>
          </div>
          <div className="pt-2 flex flex-wrap gap-4">
            <Link href="/leaderboard">
              <Button size="lg" className="rounded-full font-bold px-8 shadow-md hover:shadow-lg transition-all hover:-translate-y-0.5 bg-red-600 hover:bg-red-700 text-white">
                Sıralamayı Gör
              </Button>
            </Link>
            <Link href="/players">
              <Button variant="outline" size="lg" className="rounded-full font-bold px-8 bg-background/50 backdrop-blur hover:bg-background/80 transition-all border-red-500/30">
                Tüm Oyuncular
              </Button>
            </Link>
          </div>
          ...
          <div className="flex flex-col gap-2">
            {topPlayers.length === 0 ? (
              ...
              topPlayers.map((player: any, index: number) => {
                const rank = getRankFromMmr(player.currentMmr, "VALORANT");

                return (
                  <Link key={player.id} href={`/player/${player.nickname}?game=VALORANT`}>
                    <Card className="bg-card hover:bg-muted/50 transition-colors border-border/50 shadow-sm group">
                      <div className="p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-4 min-w-[180px] w-full sm:w-auto">
                          <div className="flex h-10 w-10 text-lg items-center justify-center rounded-full bg-red-500/10 text-red-500 font-bold shrink-0">
                            {index + 1}
                          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-3">
                ...
                <Card key={match.id} className="p-3 border-border/50 bg-card/40 backdrop-blur hover:bg-card/60 transition-all shadow-sm flex flex-col gap-3">
                  <div className="flex justify-between items-center text-xs border-b border-border/50 pb-2">
                    <div className="flex items-center gap-2">
                      ...
                      {match.season && (
                        <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-red-500/10 text-red-500 border border-red-500/20 shrink-0">{match.season.name}</span>
                      )}
                    </div>
                      ...
                      {match.participants.filter((p: any) => p.team === "BLUE").map((p: any) => (
                        <div key={p.id} className="flex items-center gap-2 w-full group">
                          ...
                          <div className="flex flex-col min-w-0">
                            <Link href={`/player/${p.player.nickname}?game=VALORANT`} className="truncate text-xs font-medium hover:underline text-muted-foreground group-hover:text-foreground transition-colors">
                              {p.player.nickname}
                            </Link>
            ...
            {recentMatches.length > 5 && (
              <Dialog>
                <DialogTrigger className="w-full py-3 text-xs font-bold tracking-wider text-red-500 bg-card/40 backdrop-blur hover:bg-card/60 transition-all shadow-sm border border-border/50 rounded-xl flex items-center justify-center gap-2 uppercase">
                  Tüm Maçları Gör ({recentMatches.length} Maç)
                </DialogTrigger>
                <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto bg-background/60 backdrop-blur-2xl border-red-500/20 shadow-2xl shadow-red-500/10 custom-scrollbar">
